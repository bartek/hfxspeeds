...
service_account_path = "hfxspeeds-709b627ea462.json"

video_client = videointelligence.VideoIntelligenceServiceClient()
storage_client = storage.Client()
bucket = storage_client.bucket(bucket_name)

# Load the service account key JSON file. This is used to authenticate with
# Google Cloud.
with open(service_account_path, 'r') as key_file:
...
# capture captures a video (input existing file) or stream and sends it to the
# annotaton service. For streams, the video is chunked and sent every 30
# minutes.
def capture(input_video):
    # Upload the input_video to Google Cloud Storage

    # Get basename of input video
    upload_blob(input_video, os.path.basename(input_video))

    # Then, annotate the video with Google Cloud Video API
    # Poll URL until ready
    # Save output when it's ready to JSON file
...
def parse_annotation(index, annotation, frame_rate, distance):
    ...
    frame_box_lookup["entrance_time"] = frames[0]["time_offset"]
    frame_box_lookup["exit_time"] = frames[-1]["time_offset"]
...
def extract_cars(annotations_response, frame_rate, distance, min_speed, min_rdt):
    blob = bucket.blob(annotations_response)
    with blob.open() as f:
        contents =  f.read()
        annotations = json.loads(contents)
        ar = annotations["annotation_results"][0]["object_annotations"]
...
def frame_to_box_lookup(frame, frame_rate):
    timeOffset = frame["time_offset"].get("seconds")
    if timeOffset is not None:
        frame_number = seconds_to_frame_number(timeOffset, frame_rate)
        bounding_box = default_bb | frame["normalized_bounding_box"]
        return (frame_number, bounding_box)
    else:
        return (None, None)

if __name__ == '__main__':
    # Capture will ensure the video is streamed or sent for annotation.
    #capture("sample_data/short.mov")
    #annotate("gs://hfxspeeds/short.mov")

    # Once we have the annotation, we can work on annotating the video. First, need to extract cars.
    # TODO: Get annotations from file
    cars_frame_lookup = extract_cars("shortannotations.json", frame_rate, distance, min_speed, min_distance)
    # TODO: This will be part of video.py
    annotate_frames(cars_frame_lookup, "short.mov", "out.mov", frame_rate, width, height)
